...
3. **中药配伍禁忌实时警戒雷达 (Milestone M1)**：
   - 内置“十八反”、“十九畏”与“妊娠用药禁忌（禁用/慎用）”结构化规则库。
   - 阅读方剂或本草条目时自动触发药味碰撞检测，提供经典歌诀出处与名家考辨，护航安全研读。

4. **古今双篇典籍分屏异文校勘工作台 (Milestone M2)**：
   - 支持分屏对照 (Split View) 与紧凑合并 (Unified View) 双模式。
   - 搭载面向古籍文本的 LCS 差分引擎，精准标注宋本、古本等不同刊本间的传抄衍脱与方剂药量增删。

5. **十二经脉流注图谱与临床对症选穴 (Milestone M3)**：
   - 完整收录十二经脉气血流注时辰、表里配偶与循行径路。
   - 支持五输穴（井荥输经合）、原穴、络穴、郄穴、俞募穴快速检索，并可根据临床症状智能推导经典要穴。
...
```text
TCM-Library-Search/
├── .github/                 # 自动化 CI / CD 工作流 (代码风格、Clippy静态分析、自动化测试、跨平台发布构建)
├── docs/                    # 架构与工程规范
...
├── crates/
│   ├── core/                # tcm_library_core 纯 Rust 检索、校雠与配伍算力内核
│   │   ├── Cargo.toml
│   │   └── src/             # 索引引擎、维度匹配器、配伍雷达、LCS异文对比、经络拓扑
│   └── desktop/             # Tauri v2 + SolidJS 桌面客户端
│       ├── package.json
│       ├── src/             # 前端 SolidJS 视图 (ReaderView, CompatibilityRadar, DiffModal, MeridianPanel)
